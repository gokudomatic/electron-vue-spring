...
function getCurrentSHA (author) {
  return new Promise((resolve, reject) => {
    ...
    get({
      host: 'api.github.com',
      path: `/repos/gokudomatic/electron-vue-spring/commits${isBranch ? '?sha=' + process.argv[2].split('#')[1] : ''}`,
      headers: {
        'User-Agent': author
      }
    }, res => {
...
function appendSHALink (sha, destDirName) {
  ...
  md = md.replace(
    ' using',
    `@[${sha.substring(0, 7)}](https://github.com/gokudomatic/electron-vue-spring/tree/${sha}) using`
  )
...
module.exports = {
  prompts: {
    ...
    plugins: {
      type: 'checkbox',
      message: 'Select which Vue plugins to install (vue-router required)',
